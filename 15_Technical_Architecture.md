...
**Further-device linking (DL-042, correction note on DL-029; term corrected per DL-045):** desktop → phone via QR code encoding a magic link (not the bare recovery code); phone → desktop via participant-self-sent mailto: magic link; desktop additionally surfaces email as a secondary path below an "oder" divider. Magic-link security requirements: single-use, expires in minutes. See correction note on DL-029 for full mechanics and rationale. **Mechanism (DL-096):** the `recovery` function issues the token (`POST /link-issue { user_id }` → 128-bit token) and redeems it (`POST /link-redeem { token }` → `user_id`, `pid`, `recovery_code`); the token lives in Catalyst Cache (segment `Default`) with the thirty-minute expiry carried in the value and is deleted on first successful redeem. The Shell reads `?ml=<token>` before `?pid=`, stores the result as after `/recover`, and renders the QR in the browser.
...
> **Correction note (2026-07-14, DL-076):** Rise 360 is dropped for the combined module. The `<iframe>`/`RiseLMSInterface` mechanics described in this section are superseded — phase content is native Markdown, loaded and rendered directly by the Shell, no iframe, no `window.RiseLMSInterface` bridge. The per-phase release-gating *principle* (date/progress-based, enforced by Shell routing logic, bundle not fetched until release) remains in force; only the thing being gated changes, from a Rise export bundle to a Markdown lesson set. The exact loader/routing/rendering mechanics for the Markdown architecture are not yet specified in this document — see DL-076, flagged there as an open build-architecture task, not invented here. Ready Check's delivery mechanism is unaffected by this correction. **Update (2026-09-05, DL-083):** those mechanics are now specified — a typed 11-block renderer, frontmatter (with `summary`), a sectioned-with-continue lesson frame (one-scroll fallback, shared renderer), section-index resume, an explicit "Lektion abschließen" completion action (DL-060), a `progress` shape filling DL-081 §6, and a data-driven loader (lesson count not hardcoded) with routing-enforced phase gating on the Markdown lesson set. This document's full technical-detail rewrite of the delivery section is tracked but not done in this pass; DL-083 is the source of truth for the mechanics.
...
> **Correction note (2026-09-05, DL-084):** for **lesson-flow reflections** the Zoho Forms embed described below is superseded — the `Reflection` block uses **native in-shell inputs** (prompt + text area, optional 1–10 scale), backed up to Catalyst `FormSubmissions` keyed by `user_id`, realigning with DL-070. The Zoho Forms embed pattern below remains accurate only for the Ready Check (the peer-group has since moved to a native Catalyst data path with ZeptoMail — DL-086; Zoho Forms is now Ready-Check-only).
...
**Not decided, flagged for build:** exact token/link format for self-exit and opt-in-growth actions; exact copy for the consent checkbox, the 3-day broadcast, and the "no group found" message. **Resolved (2026-09-08, build):** all three are built and final — bare-token links for self-exit and opt-in-growth, and reviewed copy for the consent checkbox, the broadcast and the "not enough signups" message. See DL-087 and `functions/peer/` in habify-app.

**Build update (2026-09-07, DL-086).** Data path is **native Catalyst, not Zoho Forms** — the pages POST to `/peer/enrol` and `/peer/exit-request` (Catalyst Functions), and read cohort config (domains + cutoff) from the `accesscontrol` capabilities. The operational emails (formation, exit confirmation, wait-pool, opt-in-growth) are sent via **ZeptoMail** server-to-server, at the EU endpoint `api.zeptomail.eu` (verified in the sending code). Zoho's shared-infrastructure processor characterization from DL-027 applies; what remains open is only ZeptoMail's delivery sub-processor chain (OQ-037). **Isolation is an origin boundary:** the pages ship as their own build entry deployed to their own origin, so per-origin `localStorage` isolation makes the Shell `user_id` physically unreadable there — the by-convention "no uid" boundary of DL-036/DL-053 is thereby made hard/auditable The boundary is structural only as long as it holds in both directions: the peer entry is built on its own and is the only thing deployed to the peer origin, and the Shell is deployed only to the main origin. A Shell reachable under the peer origin could write its own `h30.state` there, which would return the guarantee to a code convention. **No name field** (email + consent only). **No enrolment double opt-in** (single-step; only the exit flow confirms by email — the enrolment residual risk is accepted, see 11_Open_Questions.md). Empty `allowedEmailDomains` ⇒ client falls back to format-only validation, the Catalyst Function stays authoritative. Zoho Forms is now scoped to Ready Check only. This is the source of truth for the peer-group data/isolation mechanics; see DL-086.

**Lifecycle precisions (2026-09-08, DL-087).** **Exit is a full exit** — confirming the exit link removes the address from the signup list; the participant is *not* re-matched (supersedes DL-037's wait-pool sentence). Re-entry is an active new enrolment. **A group that drops to one member is dissolved**, and its last member receives an email with a **link** to enter the wait pool — their click, never an automatism; a 3-group dropping to 2 stays a group and gets an opt-in link with the exit notification (it had none from formation). **Wait-pool entry is therefore always an active act**, and the pool holds only late joiners plus dissolved-group members who opted back in. The **3-day bundled broadcast targets 2-person groups that are not yet open** (an open group is filled automatically, so the literal "currently-open" target is vacuous). Group partitioning is always clean for n ≥ 2 — the single-person case is a one-signup cohort or a lone solo in the pool, not a partition remainder (correction on DL-035).

**Mail frame (2026-09-08, build detail of DL-087).** All ten peer-group email artifacts share one frame, applied centrally at the send layer rather than written into each artifact: every subject is prefixed with the cohort's `programm_name`, every mail carries a footer naming `contact_email` as the route for questions, and the sender is the noreply address `noreply.habify30@k-a-d-o.com` — so the footer address is the only reply route offered. Both cohort fields are read per `pid` from `AccessControl` (DL-058; this is `contactEmail`'s first actual use, reserved without one there). Both fail open: a cohort carrying neither field still receives its mails, just without prefix and without footer.
...
* Fillout is replaced by Zoho Forms for Ready Check; Momentum and Veränderungswerkstatt inputs are native in-shell HTML; the peer-group data path is native Catalyst with ZeptoMail for its operational emails (DL-027, DL-070, DL-086).
...
* Design system foundations extended (DL-092): radius scale `0 · 4 · 6 · 8 · 12 · full` — a value between the steps is a defect, not a variation (three `Logo — habify30` frames remain at radius 5 as an accepted placeholder); `color/bg/inverse` + `color/text/on-inverse` as an inseparable pair (a surface token, explicitly not the second accent family DL-043 rejected); `Callout` is a library component, instantiated rather than rebuilt. Mobile is allowed to depart from desktop where the departure is designed: fact cards carry the icon above the text and appear as three cards rather than one container — deliberate in both Figma and code, and not to be levelled during componentisation.
...
* The Shell is deployed as its own Slate app (`shell`, source `shell/dist`) beside the peer app (`peerpages`, `shell/dist-peer`) — two apps, two origins, two builds, never one output on both (DL-086). Development since 2026-09-11 under `app-dev.habify30.k-a-d-o.com`, caching disabled, built against the Development backend (`npm run build:dev`); the peer app under `peer-dev.habify30.k-a-d-o.com` since 2026-09-08. Neither has a Production domain yet (Capabilities A5).
...
* Scheduled peer-group work — formation and wait-pool matching — runs in one Job Function (`peersweep`) under one hourly cron and nowhere else; participant-facing routes no longer trigger a match. The serialisation guarantee is the platform's 15-minute execution cap against a 60-minute interval, conditional on the interval, `retries = 0` and no manual submission during a scheduled window (DL-094). The datastore itself offers no mutual exclusion (Capabilities B8).
* Every HTTP function carries a **build marker** — a string constant in its source, returned by `GET /` (today `peer`: `{ status, message, build }`). It is bumped in the same commit as any change whose deployment has to be verifiable, and the post-deploy check reads it from the serving host. A "DEPLOYMENT SUCCESSFUL" message is not that check: warm instances keep answering with the previous build for a while, and the function's `modified_time` says when it was written, not which build answers. The marker never lives in an environment variable — a plain read of a function returns those in cleartext (Capabilities E2).
