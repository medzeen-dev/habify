...
> **Correction note (2026-09-05, DL-084):** for **lesson-flow reflections** the Zoho Forms embed described below is superseded — the `Reflection` block uses **native in-shell inputs** (prompt + text area, optional 1–10 scale), backed up to Catalyst `FormSubmissions` keyed by `user_id`, realigning with DL-070. The Zoho Forms embed pattern below remains accurate only for the Ready Check and the peer-group email (DL-070 scope).
