...
**Build update (2026-09-07, DL-086).** Data path is **native Catalyst, not Zoho Forms** — the pages POST to `/peer/enrol` and `/peer/exit-request` (Catalyst Functions), and read cohort config (domains + cutoff) from the `accesscontrol` capabilities. The operational emails (formation, exit confirmation, wait-pool, opt-in-growth) are sent via **ZeptoMail** server-to-server (EU endpoint / DPA still to confirm). **Isolation is an origin boundary:** the pages ship as their own build entry deployed to their own origin, so per-origin `localStorage` isolation makes the Shell `user_id` physically unreadable there — the by-convention "no uid" boundary of DL-036/DL-053 is thereby made hard/auditable The boundary is structural only as long as it holds in both directions: the peer entry is built on its own and is the only thing deployed to the peer origin, and the Shell is deployed only to the main origin. A Shell reachable under the peer origin could write its own `h30.state` there, which would return the guarantee to a code convention. **No name field** (email + consent only). **No enrolment double opt-in** (single-step; only the exit flow confirms by email — the enrolment residual risk is accepted, see 11_Open_Questions.md). Empty `allowedEmailDomains` ⇒ client falls back to format-only validation, the Catalyst Function stays authoritative. Zoho Forms is now scoped to Ready Check only. This is the source of truth for the peer-group data/isolation mechanics; see DL-086.
