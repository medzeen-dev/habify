...
> **Correction note (2026-09-05, DL-082):** the manual per-function CORS header does **not** cover the browser OPTIONS preflight on Catalyst — the gateway answers OPTIONS itself without the header, so cross-origin JSON `POST` is blocked. Production must use Catalyst **Authorized Domains** (gateway CORS) and remove the manual headers; Dev uses a Vite proxy (same-origin). See DL-082.
