...
| **DL-091** | **Slate-Cache bleibt aus** — für jede habify30-Slate-App, in jedem Environment, bis das System steht · Slates Default ist ein Jahr `cache-control` **auch auf `index.html`**, und Hash-Assetnamen mildern das **nicht** (sie schützen die Assets, nicht die HTML, die sie benennt) → nach einem Deploy weiße Seite bei wiederkehrenden Browsern · Steuerung: Konsole, pro Deployment, binär (`no-store` statt Jahr) · **muss vor dem ersten Besucher gesetzt sein** · korrigiert DL-068 · Messung: Capabilities **A5** |
