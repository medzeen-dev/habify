...
| **DL-086** | *Korrekturnotiz DL-090 (Double Opt-In).* **Bau der drei Seiten:** Datenweg **nativ (Catalyst), nicht Zoho Forms** · **ZeptoMail** für die operativen Mails · **Isolation = eigene Origin** (uid der Shell browserseitig unlesbar) · **kein Namensfeld**, **kein Double-Opt-In** · löst Peergruppen-Teil von DL-070 und Klarname aus DL-036 ab |
| **DL-087** | **Lebenszyklus:** **Austritt = ganz raus** (kein Wait-Pool; Wiedereintritt ist neue Anmeldung) · **1er-Gruppe wird aufgelöst**, letztes Mitglied kommt **per Link** in den Wait-Pool · **Wait-Pool-Eintritt ist immer eine aktive Handlung** · 3-Tage-Broadcast an **noch geschlossene** 2er · 3er→2er bekommt Opt-in-Link · *Korrekturnotizen an DL-035 (Partition) und DL-037* |
| **DL-090** | **Double Opt-In bei der Peergruppen-Eintragung** — eine Adresse kommt erst auf die Liste, wenn ihr Inhaber den Link in der Bestätigungsmail klickt; vorher `pending` und nie zuteilbar · Link 7 Tage gültig, unbestätigte Zeilen werden gelöscht · **hebt den DL-086-Punkt „kein Double-Opt-In" auf und kehrt OQ-031 um** · Grund ist nicht das dort abgewogene Risiko (fremde Adresse absichtlich eingetragen — selbstheilend), sondern der **Tippfehler**: eine vertippte Adresse kann existieren und einem Fremden gehören, der dann die Gruppenmail mit den Adressen der anderen bekommt · dazu ein Platz, den niemand mehr freigeben kann |
| **DL-094** | **Geplante Arbeit als Job Function `peersweep`, ein stündlicher Cron, sonst nirgends** — Formation und Matching sequenziell in einem Lauf · **Matching läuft nicht mehr beim Pool-Eintritt**, sondern nur im Sweep (bis zu 1 h Wartezeit statt Sekunden; korrigiert DL-037) · Serialisierung kommt vom **15-min-Limit gegen 60-min-Intervall**, nicht von `await` — Intervall über 15 min, retries 0, kein manueller Submit im Betrieb · kein Endpunkt (`/execute` → 403), kein `ADMIN_KEY`, keine URL im Cron · Formation stündlich statt 03:00 (idempotent per `formed_time`) · Messungen: Capabilities **B8**, **E1** |
...
| DL | Inhalt |
|---|---|
| **DL-068** | *Korrekturnotiz DL-091 (Cache).* **Catalyst Slate** als Frontend-Host (ersetzt Web Client Hosting) · SPA-Routing HTTP 200 · Root-Base-Path · Static-Framework · mehrere Apps/Projekt · Cache `max-age=31536000` auf Shell-HTML (Hash-Assets nötig) · korrigiert DL-028 |
...
| **DL-082** | **Build-Reconciliations** (erster Shell+Backend-Bau): CORS via **Authorized Domains** statt manueller Function-Header (korrigiert DL-029; Catalyst beantwortet OPTIONS am Gateway) · Ablauf über **eine `expiry_date`-Spalte** statt DL-031-Formel (verfeinert DL-058) · Ordner `Catalyst_Functions/` → `functions/` (CLI-Standard) · Spalten snake_case (`programm_name`/`contact_email`/`expiry_date`) · *Korrekturnotiz DL-089* |
| **DL-091** | **Slate-Cache bleibt aus** — für jede habify30-Slate-App, in jedem Environment, bis das System steht · Slates Default ist ein Jahr `cache-control` **auch auf `index.html`**, und Hash-Assetnamen mildern das **nicht** (sie schützen die Assets, nicht die HTML, die sie benennt) → nach einem Deploy weiße Seite bei wiederkehrenden Browsern · Steuerung: Konsole, pro Deployment, binär (`no-store` statt Jahr) · **muss vor dem ersten Besucher gesetzt sein** · Wiedereinschalten **beim ersten Kundentest**, und erst wenn der Stale-Cache-Wächter deployt ist (sonst friert man wächterlose HTML für ein Jahr ein) · korrigiert DL-068 · Messung: Capabilities **A5** |
| **DL-089** | **CORS gehört allein dem Gateway** — an genau einer Stelle konfiguriert (Authorized Domains, pro Environment), Funktionen tragen **keine** CORS-Logik · ein Function-Header ist kein Fallback, sondern ein **Duplikat**: der Gateway stempelt auch die echte Antwort, und Browser lehnen zwei `Access-Control-Allow-Origin` ab, **auch bei gleichem Wert** · gilt **auch in Development**, seit DL-086 dort ein echter Cross-Origin-Fall ist · `PEER_ORIGIN` ist damit kein CORS-Mittel mehr: bleibt auf `peer` nur für die Mail-Links, entfällt auf `accesscontrol` · korrigiert DL-082 §1 (Reichweite + Begründung) · Messung: Capabilities **E4** |

**Vor dem Bau von:** Dashboard-Queries (DISTINCT-Falle beachten), Shell-Deploy-Pipeline (Hash-Asset-Namen), **jedem Screen mit Client-State (DL-081)**, **jeder neuen Origin, die den Backend aufruft (DL-089: Authorized Domain setzen, keine Header in der Funktion)**.
