...
- Every meaningful decision gets a Decision Log entry (09_Decision_Log.md) with the sections: Decision / Context / Decision / Rationale / Consequences.
- If a decision supersedes an earlier Decision Log entry, do NOT delete the old entry. Add a correction note directly above it, in this exact format:
  `> **Correction note (YYYY-MM-DD, DL-0XX):** ...`
- Canon.md (immutable principles) only changes when a Decision Log entry explicitly requires it. Reference the DL number in the new Canon text.
...
- Standard propagation order for architecture/product decisions: 09_Decision_Log.md → Canon.md (if affected) → 03_Product_Architecture.md (if affected) → 15_Technical_Architecture.md (if affected) → Glossary.md (if affected) → 04_Business_Model.md / 11_Open_Questions.md / 12_Backlog_md.txt (if affected).
...
- Don't delete historical Decision Log entries — always add a correction note, never delete.
- Don't invent new open questions or silently resolve existing ones without an explicit instruction to do so.
- Don't reorder or renumber existing Decision Log entries, Canon principles, or Open Questions — always append.
